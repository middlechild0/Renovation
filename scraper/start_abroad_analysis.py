#!/usr/bin/env python3
"""
Abroad Comprehensive Analysis Starter
Scrapes → Analyzes → Updates database with tier assignments
Scope: Europe, USA, Canada, Australia (excludes Africa)
"""
...
# Target cities focused outside Africa
TARGET_CITIES = {
    'Europe': {
...
# Business categories to search for lucrative B2B prospects
TARGET_CATEGORIES = [
...
class AbroadAnalysisStarter:
    """Complete pipeline: scrape → analyze → store for abroad markets"""
    ...
    def run_complete_analysis(self):
        console.print(Panel(
            "[bold cyan]🚀 ABROAD BUSINESS ANALYSIS STARTER[/bold cyan]\n"
    ...
    def _analyze_city(self, city: str, city_info: dict, region: str):
        ...
        with Progress(
            ...
            for category in TARGET_CATEGORIES:
                try:
                    results = self.scraper.search_all_apis(city, category, radius=5000)
    ...
    def _print_final_summary(self, elapsed_time: float):
        ...
        for tier in ['TIER_1', 'TIER_2', 'TIER_3', 'TIER_4']:
            ...
            tier_table.add_row(f"{emoji} {tier}", f"[{color}]{count}[/{color}]", f"{pct:.1f}%")
        ...
        console.print("  → High-value leads for website builds")
        tier_2_count = self.stats['tier_distribution']['TIER_2']
        if tier_2_count > 0:
            console.print(f"\n  🔴 {tier_2_count} businesses with CRITICAL ISSUES")
            console.print("  → Prioritize SSL/mobile/contact fixes")
